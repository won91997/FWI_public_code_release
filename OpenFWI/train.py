...
# 公平模式：需从 repository 根目录导入 benchmark
...
def save_val_visualization(model, dataloader, device, save_path):
    model.eval()
    with torch.no_grad():
...
def main(args):
    ...
    ctx = None
    if not args.use_unified_loader:
    ...
    if args.use_unified_loader:
        dataset_train, dataset_valid = build_unified_dataset(args)
    else:
    ...
    # 公平模式：InversionNet 入口下采样 256->70，出口上采样 70->256
    if os.environ.get('BENCHMARK_FAIR_MODE') == '1':
        ...
        if args.rank == 0 or not args.distributed:
            print('🔥 [FairMode] Wrapping InversionNet (256 -> 70 -> 256)')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.vis_interval > 0 and (epoch + 1) % args.vis_interval == 0:
            vis_path = os.path.join(args.output_path, "visualizations", f"epoch_{epoch+1:04d}.png")
            save_val_visualization(model, dataloader_valid, device, vis_path)
            print(f"saving validation visualization at epoch: {epoch + 1}")
...
def parse_args():
    ...
    parser.add_argument('-eb', '--epoch_block', type=int, default=5, help='epochs in a saved block')
    ...
    parser.add_argument('--vis-interval', default=5, type=int,
        help='save val prediction figure every N epochs')
    parser.add_argument('--use-unified-loader', action='store_true',
        help='Use benchmark/unified_loader.py instead of txt annotations')
    parser.add_argument('--data-root', default=os.environ.get('DATA_ROOT'),
        help='dataset root used by UnifiedFWIDataset')
    parser.add_argument('--global-map-csv', default='../benchmark/generated_split/global_map.csv',
        help='global_map.csv generated by make_split_from_inline.py')
    parser.add_argument('--stats-json', default='../benchmark/generated_split/train_stats.json',
        help='train-only stats json generated by compute_train_stats.py')
    parser.add_argument('--time-downsample', default=1, type=int,
        help='temporal downsample rate for UnifiedFWIDataset')
    parser.add_argument('--channel-mode', default='all', choices=['all', 'middle'],
        help='channel selection mode for UnifiedFWIDataset')
    parser.add_argument('--align-multiple', default=1, type=int,
        help='align time/width to multiple, e.g. 32')
    parser.add_argument('--align-mode', default='none', choices=['none', 'crop', 'pad'],
        help='shape alignment mode for unified loader')
    parser.add_argument('--target-time', default=0, type=int,
        help='explicit target time length after downsample; 0 means auto')
    parser.add_argument('--target-width', default=0, type=int,
        help='explicit target width; 0 means auto')
    parser.add_argument('--seed', type=int, default=None,
        help='random seed (overrides BENCHMARK_SEED env var)')
    parser.add_argument('--output-height', type=int, default=256,
        help='model output height for resolution-agnostic architecture')
    parser.add_argument('--output-width', type=int, default=256,
        help='model output width for resolution-agnostic architecture')
