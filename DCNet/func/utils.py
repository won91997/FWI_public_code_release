...
def pain_seg_seismic_data(para_seismic_data, is_colorbar = 1): #绘制SEG盐体地震数据的图像
    '''
    ...
    if is_colorbar == 0:
        ...
        fig, ax = plt.subplots(figsize=(6.2, 8), dpi = 120) #Figure的宽度和高度6.2*8 分辨率为120

    im = ax.imshow(para_seismic_data, extent=[0, 300, 400, 0], cmap=plt.cm.seismic, vmin=-0.4, vmax=0.44) # 表示图像的左、右、下、上边界。在这里，x轴的范围是从0到300，y轴的范围是从0到400

    ax.set_xlabel('Position (km)', font21)
    ax.set_ylabel('Time (s)', font21)

    ax.set_xticks(np.linspace(0, 300, 5)) #x轴的刻度位置
    ax.set_yticks(np.linspace(0, 400, 5)) #y轴的刻度位置
    ax.set_xticklabels(labels = [0,0.75,1.5,2.25,3.0], size=21) #这一行设置了x轴刻度位置对应的标签
...
def pain_openfwi_velocity_model(para_velocity_model, min_velocity, max_velocity, save_path, save_name, is_colorbar = 1):
    '''
    Plotting seismic data images of openfwi dataset
    ...
    if is_colorbar == 0:
        ...
        pathName = save_path + save_name + '.jpg'
        plt.savefig(pathName)

    plt.show()

def pain_seg_velocity_model(para_velocity_model, min_velocity, max_velocity, save_path, save_name, is_colorbar = 1):
    '''
...
def extract_contours(para_image): #para_image模型的图像，通常是一个二维的数组（矩阵）
    '''
    ...
    norm_image = cv2.normalize(image, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F) #函数对输入图像进行规范化
    norm_image_to_255 = norm_image * 255
    norm_image_to_255 = norm_image_to_255.astype(np.uint8)
    canny = cv2.Canny(norm_image_to_255, 10, 15)  #对图像进行Canny边缘检测
    bool_canny = np.clip(canny, 0, 1) #函数将Canny检测结果二值化，将所有非零像素值设置为1，以获得二进制的轮廓结构，然后返回该二进制轮廓图像
    return bool_canny
